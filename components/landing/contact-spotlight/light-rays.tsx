...
/** Atributos alineados con `ogl` Renderer (alpha true como en este componente). Evita `console.error` de ogl si no hay GL. */
const LIGHT_RAYS_GL_ATTRIBUTES: WebGLContextAttributes = {
  alpha: true,
  depth: true,
  stencil: false,
  antialias: false,
...
function createRendererIfWebGLAvailable(dpr: number): Renderer | null {
  const canvas = document.createElement("canvas")
  let gl = canvas.getContext("webgl2", LIGHT_RAYS_GL_ATTRIBUTES)
  const isWebgl2 = !!gl
  if (!gl) gl = canvas.getContext("webgl", LIGHT_RAYS_GL_ATTRIBUTES)
  if (!gl) return null
...
const LightRays: React.FC<LightRaysProps> = ({
  ...
  useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver((entries) => setIsVisible(entries[0].isIntersecting), {
      threshold: 0.1,
    })
  ...
  useEffect(() => {
    ...
    const initializeWebGL = () => {
      ...
      try {
        const created = createRendererIfWebGLAvailable(Math.min(window.devicePixelRatio, 2))
        if (!created) return
        renderer = created
      } catch {
        return
      ...
      const unsubX = dynamicOrigin?.x.on("change", (v: number) => {
        if (uniformsRef.current) uniformsRef.current.rayPos.value[0] = v * renderer.dpr
      })
      const unsubY = dynamicOrigin?.y.on("change", (v: number) => {
        if (uniformsRef.current) uniformsRef.current.rayPos.value[1] = v * renderer.dpr
      })
