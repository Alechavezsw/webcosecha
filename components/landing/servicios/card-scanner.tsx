...
/** Misma copy que la sección #beneficios en la landing IA (Cosecha Creativa) */
export const IA_BENEFITS = [
  {
    title: "Equipos más potentes",
    body: "La IA no reemplaza personas: libera tiempo para que vendan, creen y resuelvan mejor.",
  },
  {
    title: "Menos fricción operativa",
    body: "Menos copiar y pegar entre Excel, mail y WhatsApp. Más continuidad entre herramientas.",
  },
  {
    title: "Decisiones con contexto",
    body: "Datos ordenados y respuestas accionables en lugar de reportes que nadie lee.",
  },
  {
    title: "Experiencia de cliente más ágil",
    body: "Respuestas más rápidas y coherentes sin multiplicar headcount en horarios imposibles.",
  },
] as const
...
type CardScannerProps = {
  /** En landing IA: altura acotada y ancho al contenedor (no pantalla completa) */
  embedded?: boolean
  className?: string
}

export function CardScanner({ embedded = false, className }: CardScannerProps) {
  ...
  useEffect(() => {
    ...
    class CardStreamController {
      ...
      generateBenefitAscii(benefit: (typeof IA_BENEFITS)[number], width: number, height: number): string {
        const chunk = `${benefit.title} · ${benefit.body} `
        let flow = chunk.repeat(40).replace(/\s+/g, " ").trim()
        while (flow.length < width * height + width * 2) {
          flow += " " + chunk
        }
        let out = ""
        let offset = 0
      ...
      createCardWrapper(index: number): HTMLDivElement {
        ...
        const benefitIndex = index % IA_BENEFITS.length
        wrapper.dataset.benefitIndex = String(benefitIndex)

        const benefit = IA_BENEFITS[benefitIndex]
        ...
        normalCard.innerHTML = `
          <div class="benefit-card-inner">
            <span class="benefit-chip">Beneficios · IA</span>
            <h3 class="benefit-title">${escapeHtml(benefit.title)}</h3>
            <p class="benefit-body">${escapeHtml(benefit.body)}</p>
          </div>
        `
        ...
        asciiContent.textContent = this.generateBenefitAscii(benefit, width, height)
      ...
      updateAsciiContent() {
        document.querySelectorAll(".ascii-content").forEach((content) => {
          ...
          const idx = Number(wrapper?.getAttribute("data-benefit-index") ?? 0)
          const benefit = IA_BENEFITS[idx % IA_BENEFITS.length]
          if (Math.random() < 0.15) {
            const { width, height } = this.calculateCodeDimensions(400, 250)
            content.textContent = this.generateBenefitAscii(benefit, width, height)
          }
        })
      }
  ...
  100% {
    ...
    opacity: 0;
      ...
      <div
        className={cn(
          "card-scanner-container",
          embedded && "card-scanner-container--embedded",
          className,
        )}
        ref={containerRef}
      >
